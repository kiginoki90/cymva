...
本プライバシーポリシー（以下、「本ポリシー」といいます。）は、cymva（以下、「本サービス」といいます。）を運営する本サービス提供者（以下、「当スタジオ」といいます。）が、ユーザーの個人情報をどのように収集、利用、管理するかを定めたものです。
          '''),
            _buildSectionTitle('第1条（収集する個人情報）'),
            _buildSectionContent('''
当スタジオが収集する個人情報は以下のとおりです：
...
- 広告配信や効果測定のため（匿名情報のみ）
            ...
            _buildSectionContent('''
当スタジオは、ユーザーの個人情報を適切に管理し、不正アクセス、紛失、破損、改ざん、漏洩などを防止するために、以下の措置を講じます：
- SSL（Secure Socket Layer）による通信の暗号化
- 個人情報へのアクセス制限
            '''),
            _buildSectionTitle('第4条（第三者への提供）'),
            _buildSectionContent('''
当スタジオは、個人データについてあらかじめ利用者の同意を得ることなく、第三者に提供いたしません。ただし、次に掲げる場合はその限りではありません：
　（1）法令に基づき開示が必要な場合
...
ユーザーは、デバイス設定を変更することで、クッキーや広告IDの利用を制限または無効化することができます。ただし、その場合、一部の機能が利用できなくなる可能性があります。
            ...
            _buildSectionContent('''
当スタジオは、利用者情報の取り扱いについて適宜見直し継続的な改善を努めるものとし、必要に応じて改定される場合があります。重要な変更がある場合は、本サービス上で通知いたします。改定後のポリシーは、本サービスに掲載した時点から効力を生じます。なお、法令上利用者の同意が必要とされるような内容の変更の場合は、改定後のポリシーをご確認いただいた上で、利用者の同意を得るものとします。
            '''),
